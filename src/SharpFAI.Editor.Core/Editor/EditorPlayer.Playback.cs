...
public partial class EditorPlayer : IPlayer
{
    ...
    {
        // CreatePlayer 逻辑已在 LoadLevel 中实�?
    ...
    public void UpdatePlayer(double delta)
    {
        ...
        if (_isPlaying)
        {
            ...
            {
                totalTime = _noteTimes[_noteTimes.Count - 1] / 1000.0 + 5.0; // 最后一个音�?+ 5秒缓�?
            }
        ...
        if ( _isPlaying && (_hitSound?.IsPlaying ?? false))
        {
    ...
    {
        if (!_initialized || _level == null || _isPlaying)
            return;
        ...
        _showPlanets = true;
        _angle = 0;
        await Task.Delay(TimeSpan.FromMilliseconds(60000 / _level.GetSetting<double>("bpm") * 4));
        // 如果有音乐，启动音频
        ...
        if (_music != null || _hitSound != null)
        {
            ...
            /*_music?.PlayAfterAsync((offset - 2) / 1000.0 ,() =>
            {
                _hitSound?.Play();
                _isPlaying = true;
            });*/
