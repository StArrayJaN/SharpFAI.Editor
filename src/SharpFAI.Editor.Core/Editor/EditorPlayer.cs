...
public partial class EditorPlayer : GameWindow
{
    ...
    protected override void OnRenderFrame(FrameEventArgs args)
    {
        ...
        if (_imGuiController != null)
        {
            try
            {
                // ImGui.Update 已经�?OnUpdateFrame 中调�?
    ...
    private async void LoadLevel(string? levelPath)
    {
        try
        {
            ...
            _statusMessage = "正在初始化音�?.. Initializing audio...";
            try
            {
                // 尝试获取音频路径
                ...
                {
                    _statusMessage = "未找到音频文件，将以无音乐模式运�?No audio file found, running without music";
                }
            }
            catch (Exception e)
            {
            ...
            _statusMessage = $"加载完成！共 {_floors.Count} 个地板，正在初始化渲�?..";
        }
        catch (Exception ex)
        {
    ...
    private void InitializeGLObjects()
    {
        ...
        try
        {
            ...
            _needRenderOrderUpdate = true; // 标记需要更新渲染顺�?
            
            _needsGLInitialization = false;
            _initialized = true;
            _statusMessage = $"加载完成！共 {_floors.Count} 个地板，按空格播�?Press Space to play";
        }
        catch (Exception ex)
        {
            _statusMessage = $"OpenGL 初始化失�? {ex.Message}";
    ...
    private void SaveLevel()
    {
        ...
        try
        {
            ...
            _statusMessage = $"已保�? {Path.GetFileName(_levelPath)}";
        }
        catch (Exception ex)
        {
    ...
    private void SaveLevelAs()
    {
        ...
        if (!string.IsNullOrEmpty(filePath))
        {
            ...
            {
                _statusMessage = $"另存为失�? {ex.Message}";
